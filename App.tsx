...
const App: React.FC = () => {
  ...
  const refreshPrices = useCallback(async (force: boolean = false) => {
    ...
    try {
      const result = await fetchRealTimePrices(tickersToUpdate, force);
      if (result) {
        const priceMap: Record<string, MarketPrice> = {};
  ...
  useEffect(() => {
    if (!isLoading && tickersToUpdate.length > 0 && !autoRefreshRef.current) {
      const doRefresh = async () => {
         ...
         if (Object.keys(marketPrices).length === 0) await refreshPrices(false);
      };
  ...
  const saveSettings = () => {
    ...
    // Força atualização imediata para testar a nova URL e atualizar dados
    if (customApiUrl && tickersToUpdate.length > 0) refreshPrices(true);
  };
